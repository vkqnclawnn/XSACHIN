document.addEventListener('DOMContentLoaded', () => {
    ...
    const questions = [
        { // 1. 가장 일반적인 소통
            ...
            options: [
                { text: "전혀 상관없다", score: 10 },
                { text: "가끔은 괜찮다", score: 8 },
                { text: "자주는 좀 그렇다", score: 5 },
                { text: "왠만하면 안 했으면 좋겠다", score: 2 },
        ...
        { // 2. 고민 상담
            ...
            options: [
                { text: "얼마든지 괜찮다", score: 10 },
                { text: "필요하다면 그럴 수 있다", score: 8 },
                { text: "가끔은 괜찮지만, 나한테 먼저 말했으면 좋겠다", score: 5 },
                { text: "왠만하면 나하고만 했으면 좋겠다", score: 2 },
                { text: "절대 안 된다", score: 0 }
            ]
        },
        { // 3. 별명 사용
            ...
            options: [
                { text: "전혀 신경 안 쓴다", score: 10 },
                { text: "그럴 수 있다고 생각한다", score: 8 },
        ...
        { // 7. 단둘이 영화
            ...
            options: [
                { text: "매우 괜찮다", score: 10 },
                { text: "친한 사이면 그럴 수 있다", score: 8 },
                { text: "썩 내키지는 않는다", score: 5 },
                { text: "왠만하면 안 갔으면 좋겠다", score: 2 },
                { text: "절대 안 된다", score: 0 }
            ]
        },
        { // 8. 커플룩 의심
            ...
            options: [
                { text: "전혀 신경 안 쓴다, 우연일 수도 있다", score: 10 },
                { text: "신경은 쓰이지만 그럴 수도 있다", score: 8 },
                { text: "썩 기분 좋지는 않다", score: 5 },
                { text: "절대 안 된다고 생각한다", score: 2 },
        ...
        { // 9. 휴대폰 배경화면
            ...
            options: [
                { text: "전혀 상관없다", score: 10 },
                { text: "단체 사진이면 괜찮다", score: 8 },
                { text: "단둘이 찍은 거라면 좀 그렇다", score: 5 },
                { text: "절대 안 된다", score: 2 },
        ...
        { // 10. 데이트 약속 변경
            ...
            options: [
                { text: "정말 중요한 일이면 그럴 수 있다", score: 10 },
                { text: "가끔은 그럴 수 있다", score: 8 },
    ...
    function getResultSummary(score) {
        // 총 10문제, 문제당 최대 10점, 총점 100점 기준
        if (score >= 80) return `1단계: 완전 쿨내 진동! 내 사람 믿어주는 당신의 연인
"어? 남사친이랑 논다고? 응, 재미있게 놀다 와! 나도 내 할 일 할게!"

당신은 정말이지 대인배 스타일이네요. 남사친, 여사친 존재를 당연하게 받아들이고,어떤 친구를 만나든 전적으로 믿어줘요. 어쩌면 관계에 대한 확신이 엄청나거나, 각자의 사생활을 존중하는 게 중요하다고 생각하는 사람일 거예요. 당신같은 이런 연인이라면 마음 편하게 친구들을 만날 수 있겠죠?`; // 80% 이상
        if (score >= 60) return `2단계: 어라? 갑자기 촉이 오네? 살짝은 신경 쓰이는 당신의 연인
"오늘 누구 만났어? 아, 그 친구? 잘 놀았어? 히히."

대놓고 질투하는 건 아닌데, 남사친이나 여사친을 만났다고 하면 괜히 한 번 더 물어보는 스타일이에요.혹시나 하는 마음에 살짝 궁금해하는 정도랄까요? 귀여운 질투라고 할 수 있죠! 당신이 애인을 너무 좋아해서 생기는 자연스러운 반응이니, 예쁘게 봐주세요!`;       // 60% - 79%
        if (score >= 40) return `3단계: 나도 모르게 시선 강탈! 왠지 모르게 불편한 당신
"음... 그 친구랑은 자주 만나는 편이야? 뭔가 둘이 분위기가 좀 다른 것 같기도 하고..."

이쯤 되면 당신의 눈에는 X사친의 존재가 살짝 거슬리기 시작할 거예요. 대놓고 불평하진 않아도, 연이이 그들과 함께 있을 때의 모습을 좀 더 유심히 보거나, 만남 후의 대화 내용을 파고들 수도 있어요. 아직은 꾹 참고 있지만, 속으로는 연인의 주변 이성에 대한 경계심이 스멀스멀 올라오고 있을지도 몰라요.`;   // 40% - 59%
        if (score >= 20) return `4단계: 선 넘지 마라. 이건 좀 아닌데? 꽤나 질투하는 당신
"솔직히 그 친구랑 너무 자주 연락하는 거 아니야? 내가 좀 신경 쓰여. 왜 나 말고 그 친구랑 그렇게 친해?"

이 단계부터는 연인의 질투가 꽤나 직접적으로 드러나요. 연인의 X사친과의 관계에 대해 노골적으로 불만을 표현하거나, 연인에게 조심하라고 이야기할 수도 있어요. 연인을 너무 사랑해서 다른 이성과 관계를 못마땅하게 보는 건데, 이때는 서로 마음을 좀 헤아려 줄 필요가 있어요 친구 관계도 중요하지만, 연인의 마음도 소중하니까요!`; // 20% - 39%
        return `5단계: 내 옆엔 오직 나뿐! 레전드 질투심을 보이는 당신
"도대체 그 친구가 왜 자꾸 너한테 연락하는 거야? 나랑 만나면서 굳이 그 친구랑 볼 필요 있어? 앞으로 그 친구 만나지 마."

아마도 당신은 연인을 향한 독점욕이 엄청나게 강할 거예요. X사친과의 모든 교류를 극도로 싫어하고, 친구 관계에 대놓고 개입하려 할 수 있어요. 심지어 연락 자체를 못하게 하거나, 특정 친구를 만나지 말라고 요구할 수도 있죠. 연인을 너무 사랑해서 그런 거겠지만, 이런 경우에는 연인과 솔직하고 진지하게 대화해서 서로의 선을 정하는 게 정말 중요해요.`;                     // 20% 미만
    }
